...
define(["jquery", "app/logger", "settings"], function(jQuery, logger, settings) {
    var settings = settings;
    
    var mapView = {
      map: [],
      default_lat: 50.0830986,
      default_lng: 6.7613762,
      user_index: 0,
      emptyDex: [],
      forts: [],
      info_windows: [],
      ...
      setBotPathOptions: function(checked) {
          var self = this;
          for (var i = 0; i < settings.users.length; i++) {
            self.user_data[settings.users[i]].trainerPath.setOptions({
      ...
      bindUi: function() {
        ...
        $('#strokeOn').prop('checked', settings.botPath);

        $('#switchPan').change(function() {
          if (this.checked) {
            settings.userFollow = true;
          } else {
            settings.userFollow = false;
          }
        });

        $('#switchZoom').change(function() {
          if (this.checked) {
            settings.userZoom = true;
          } else {
            settings.userZoom = false;
          }
        });

        $('#strokeOn').change(function() {
          settings.botPath = this.checked;
...
//        setInterval(self.addInventory, 5000);
      ...
      addCatchable: function() {
        var self = mapView;
        for (var i = 0; i < settings.users.length; i++) {
          self.loadJSON('catchable-' + settings.users[i] + '.json', self.catchSuccess, self.errorFunc, i);
        }
      },
      addInventory: function() {
        var self = mapView;
        for (var i = 0; i < settings.users.length; i++) {
          self.loadJSON('inventory-' + settings.users[i] + '.json', self.invSuccess, self.errorFunc, i);
        }
      },
      buildMenu: function(user_id, menu) {
        ...
        switch (menu) {
          case 1:
            var current_user_stats = self.user_data[settings.users[user_id]].stats[0].inventory_item_data.player_stats;
            ...
            break;
          case 2:
            var current_user_bag_items = self.user_data[settings.users[user_id]].bagItems;
        var total = 0;
            $('#sortButtons').html('');
            ...
            break;
          case 3:
            var pkmnTotal = self.user_data[settings.users[user_id]].bagPokemon.length;
            ...
            break;
          case 4:
            var pkmnTotal = self.user_data[settings.users[user_id]].pokedex.length;
      ...
      catchSuccess: function(data, user_index) {
        var self = mapView,
          user = self.user_data[settings.users[user_index]],
          poke_name = '';
        if (data !== undefined && Object.keys(data).length > 0) {
          ...
          if (data.latitude !== undefined) {
            if (user.catchables.hasOwnProperty(data.spawnpoint_id) === false) {
              ...
              logger.log({
                message: "[" + settings.users[user_index] + "] " + poke_name + " appeared",
                color: "green-text"
              });
          ...
          if (user.catchables !== undefined && Object.keys(user.catchables).length > 0) {
            logger.log({
              message: "[" + settings.users[user_index] + "] " + poke_name + " has been caught or fled"
            });
      ...
      findBot: function(user_index) {
        var self = this,
          coords = self.pathcoords[settings.users[user_index]][self.pathcoords[settings.users[user_index]].length - 1];
      ...
      getCandy: function(p_num, user_id) {
        var self = this,
          user = self.user_data[settings.users[user_id]];
      ...
      invSuccess: function(data, user_index) {
        var self = mapView,
          userData = self.user_data[settings.users[user_index]],
        ...
        self.user_data[settings.users[user_index]] = userData;
      },
      pad_with_zeroes: function(number, length) {
         ...
         * TRAINER CODE
         */
        placeTrainers: function() {
            var self = this;

            for (var i = 0; i < settings.users.length; i++) {
                var user_index = settings.users[i];

                self.user_data[user_index] = {};

                self.buildTrainerList();
//                self.addInventory();
                logger.log({
                    message: "Trainer loaded: " + user_index,
                    color: "blue-text"
                });

                var randomSex = Math.floor(Math.random() * 1);
                self.user_data[user_index].marker = new google.maps.Marker({
                    map: self.map,
                    position: {
                        lat: self.default_lat,
                        lng: self.default_lng
            ...
            if (settings.users.length === 1 && settings.userZoom === true) {
                self.map.setZoom(settings.zoom);
            }
            if (settings.users.length === 1 && settings.userFollow === true) {
                self.map.panTo({
                    lat: self.default_lat,
                    lng: self.default_lng
                });
        ...
        updateTrainerPosition: function(user_id, coords){
            var self = this;

            var user_index = settings.users.indexOf(user_id)
            if(user_index == -1){
                return;
            }

            self.pathcoords[user_id].push({
              lat: parseFloat(coords.lat),
              lng: parseFloat(coords.lng)
            });

            self.user_data[user_id].marker.setPosition({
              lat: parseFloat(coords.lat),
              lng: parseFloat(coords.lng)
            });
            if (self.pathcoords[user_id].length === 2) {
              ...
              self.user_data[user_id].trainerPath.setPath(self.pathcoords[user_id]);
            }
            self.setBotPathOptions(settings.botPath);
      ...
      trainerFunc: function(data, user_index) {
        ...
        if (coords > 1) {
          ...
          if (tempcoords.lat != coords.lat && tempcoords.lng != coords.lng || self.pathcoords[settings.users[user_index]].length === 1) {
            self.pathcoords[settings.users[user_index]].push({
              lat: parseFloat(data.lat),
              lng: parseFloat(data.lng)
            });
          }
        ...
        if (self.user_data[settings.users[user_index]].hasOwnProperty('marker') === false) {
          self.buildTrainerList();
          self.addInventory();
          logger.log({
            message: "Trainer loaded: " + settings.users[user_index],
            color: "blue-text"
          });
          var randomSex = Math.floor(Math.random() * 1);
          self.user_data[settings.users[user_index]].marker = new google.maps.Marker({
            map: self.map,
            position: {
              lat: parseFloat(data.lat),
              lng: parseFloat(data.lng)
            ...
            icon: 'image/trainer/' + self.trainerSex[randomSex] + Math.floor(Math.random() * self.numTrainers[randomSex]) + '.png',
            zIndex: 2,
            label: settings.users[user_index],
            clickable: false
          });
        } else {
            self.updateLocation(user_index, {
                lat: data.lat,
                lng: data.lng
            })
        }
      ...
      updateTrainer: function() {
        var self = mapView;
        for (var i = 0; i < settings.users.length; i++) {
          self.loadJSON('location-' + settings.users[i] + '.json', self.trainerFunc, self.errorFunc, i);
        }
      },
