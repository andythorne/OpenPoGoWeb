...
$(document).ready(function() {
  ...
  socket_io = io.connect('http://' + document.domain + ':' + location.port + '/event');
    ...
    socket_io.on('position', function(msg) {
      for(var i = 0; i < msg.length; i++) {
        mapView.updateLocation(msg[i].user, {
            lat: msg[i].coords[0],
            lng: msg[i].coords[1]
        })
      }
...
var mapView = {
  ...
  init: function() {
    var self = this;
    self.settings = $.extend(true, self.settings, set);
    self.bindUi();
  ...
  placeTrainer: function() {
    ...
    if (self.settings.web_sockets == true){
  ...
  updateLocation: function(user_id, coords){
